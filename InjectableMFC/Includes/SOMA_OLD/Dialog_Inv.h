...
public:
	...
	void SendPlayerRepairItem(short index);
	void CaptureScrollPos()
    {
		m_iCurScrollPos = m_pScrollBar->GetStartLine();
    }

	bool IsRepairActive();

    void ResetRepairItem()
    {
		m_sRepairItem = -1;
		m_bRepairActive = false;
    }
...
//	CUIScrollBar	*m_pScrollBar;
	CItemDataMap	m_mapItem;
	CItemData		*m_pArmItem[ARM_COUNT];
	...
	// PLAYER REPAIR VARIABLES

	// The item index of the item that gives the user the repair ability
	// -1 = default and Special Repair(requiring no item) any other value in here is an
	// item such as a Grind Stone
	short           m_sRepairItem;

	// Whether the user has clicked the Grind Stone or SR and the client should therefor
	// display a hammer cursor and Repair Info.
	bool            m_bRepairActive;
};
