...
public:
	...
	static	void	SetDInt(PBYTE bypData, int &offset, QWORD data);
...
public:
	...
	void Special_RailProcess(PBYTE bypData);
	void Special_CircleProcess(PBYTE bypData);
	...
	void EncryptionStartResult(PBYTE bypData);
	...
	void EffectProcess(PBYTE bypData);
	void SetRunMode(PBYTE bypData);
	void SendMonsterSelect();
	void ResurrectionScrollProcess(PBYTE bypData); // Shows magic effect and removes scroll item
	void PartyLocation(PBYTE bypData);
...
inline void CPacketParser::SetDInt(PBYTE bypData, int &offset, QWORD data)
{
	SocketMng::MP_AddDInt(bypData, offset, data);
}
