...
public:
	...
	virtual ~DataPack(){SAFE_DELETE(m_pData);}
...
public:
	...
	void			Encode_Decode2(LPBYTE lpTarget, LPBYTE lpSource, WORD nLen);
	...
	void			SendDE();
	void			SendRest();
	bool			SendDEWait(int DEDelay);
	bool			Is_InGame;
	DWORD			Delay;
	DWORD			m_wTick;
	CUser*			m_pInGame;
	DWORD			m_dwTick;
	void			SendEncryptionReq(); // Packet Encryption Start Request!
	...
	static	void	MP_AddDInt(BYTE *dest, int &offset, QWORD dword);
...
inline void SocketMng::MP_AddDInt(BYTE *dest, int &offset, QWORD dword)
{
	CopyMemory(dest+offset, &dword, 8);
	offset += 8;
}
