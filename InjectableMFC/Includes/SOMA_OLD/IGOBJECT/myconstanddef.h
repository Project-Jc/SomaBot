...
typedef struct _ANI_HEADER_SIMPLE
{
	//char	szID[4];		// 'A' 'N' 'I' 방향 숫자(1,8) 등등
	//char	szRemark[64];	// 주석
	short	nDirQt;			// 방향
	short	nQt;			// Animation 숫자

	_ANI_HEADER_SIMPLE const & operator = (_ANI_HEADER const & other) {
		//memcpy(this, &other, sizeof(*this));
		nDirQt = other.nDirQt;
		nQt = other.nQt;
		return *this;
	}
	_ANI_HEADER_SIMPLE const & operator = (_ANI_HEADER_SIMPLE const & other) {
		memcpy(this, &other, sizeof(*this));
		return *this;
	}
} _ANI_HEADER_SIMPLE;
...
typedef struct _SPR_HEADER_SIMPLE
{
	char	szID[4];		// 'SPLN' 보통 LSP Sprite | 'SPLC' 확장된 LSP Sprite || 'SPRN' 보통 Sprite
	//char	szRemark[64];	// 주석
	//char	szBMPFN[64];	// 비트맵 파일 이름 - 데이터를 갖고 있으면 안 쓴다.
	...
	//COLORREF SrcCK;			// 소스 컬러키
	int		nQt;			// 스프라이트 수
	_SPR_HEADER_SIMPLE const & operator = (_SPR_HEADER const & other)
	{
		//memcpy(this, &other, sizeof(*this));
		memcpy(szID, other.szID, 4);
		nWidth = other.nWidth;
		nHeight = other.nHeight;
		nQt = other.nQt;
		return *this;
	};
	_SPR_HEADER_SIMPLE const & operator = (_SPR_HEADER_SIMPLE const & other)
	{
		memcpy(this, &other, sizeof(*this));
		return *this;
	};
...
typedef struct _TILE_SET_HEADER_SIMPLE
{
	//char	szName[64];
	char	szBMPFN[64];
	//COLORREF crCK; // 컬러키..
	short int		nQtSubSet; // 타일셋 갯수
} _TILE_SET_HEADER_SIMPLE;//, *LPTILE_DATA_TOOL;
...
const int MAX_MAP_OBJ_REG_EX = 127;
typedef struct _MAP_OBJ_HEADER_EX
{
	char	szID[4]; // 'M' 'P' 'O' 'D'
	char	szRemark[64]; // 주석
	int		nWidth; // 너비
	int		nHeight; // 높이
	char	szRegObjAniFNs[MAX_MAP_OBJ_REG_EX][64];
	char	szRegObjSprFNs[MAX_MAP_OBJ_REG_EX][64];
	_MAP_OBJ_HEADER_EX const & operator = (_MAP_OBJ_HEADER_EX const & other)
	{
		memcpy(this, &other, sizeof(*this));
		return *this;
	}

	_MAP_OBJ_HEADER_EX const & operator = (_MAP_OBJ_HEADER const & other)
	{
		...
		for (int i=0; i<MAX_MAP_OBJ_REG; i++) {
			memcpy(szRegObjAniFNs[i], other.szRegObjAniFNs[i], 64);
			memcpy(szRegObjSprFNs[i], other.szRegObjSprFNs[i], 64);
		}
		return *this;
	}
