...
#define  ANI_FRAME_COUNT	4

class CItem_Inv  
{
public:
	void LoadAni(LPCTSTR szAniFileName, bool bDevil);
	void Load_spl(LPSTR szFileName, int type, int file_index, int arrindex, bool bDevilItem);// select는 투구인지 갑옷인지 검인지를 구분..각각 서페이스를 따르게 가지고있으므로..
	void Init(int Sex=0, bool bDevil = false);
	CItem_Inv();
	virtual ~CItem_Inv();
...
public:	
	...
	void Draw_Obj(int tmp_x = 0, int tmp_y = 0, CObjLSP* pObj = NULL);
	...
	void		 BltLSPNormal(CDDSurface *pSurface, int x, int y, RECT* pRC, BYTE* data, WORD* pPallete);
	int          m_SkinColor;
...
inline void CItem_Inv::PutPixelTrans(const WORD* pDest, const WORD* pSrc, int nCopyCount, int alpha, DWORD dwRBBitMask,DWORD dwGBitMask)
{
	...
	DWORD dwRGBBitMask = dwRBBitMask | dwGBitMask;
...
COPY_LOOP:
		...
		and ecx, dwRGBBitMask;
		
		cmp ecx, dwRBBitMask; // Test if pixel is an invisible pixel...
		je DONT_COPY;
	//	jecxz DONT_COPY; // jump if pixel is 0
